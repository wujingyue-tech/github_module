...
在 `lib/app/router.dart` 加 `GoRoute`。需要登录的页面，现有 redirect 会挡住未登录用户。冷启动是系统 Launch Screen（`LaunchScreen.storyboard`，由 `flutter_native_splash:create` 生成），不要再加 Flutter `/splash` 页。桌面图标是 Invertocat，闪屏是 GitHub 字标，不要两套都用同一只猫。改图后需要整次 `flutter run`，卸载不够。
...
- 页面：override 端口后 `pump` 空态 / 错误 / 列表 / 登录跳转。对照 `test/repo_list_page_test.dart`、`test/login_page_test.dart`、`test/profile_page_test.dart`、`test/settings_page_test.dart`、`test/router_test.dart`。不要为了测 UI 去假 Notifier。
